...
podman run \
  -v /home/redhat/tmp/data:/usr/vse/data:Z \
  quay.io/redhat-partner-solutions/vse-sync-test:latest ./cmd/e2e.sh
----
== MNO Support
...
- Clone the monorepo and create a `data/` directory alongside it:
[source,shell]
----
$ tree -L 1
.
