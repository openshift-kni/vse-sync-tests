...
* https://github.com/openshift-kni/vse-sync-tests/blob/main/tests/sync/G.8272/time-error-in-locked-mode/DPLL-to-PHC/PRTC-A/testimpl.py[reference implementation], a symbolic link
* https://github.com/openshift-kni/vse-sync-tests/blob/main/tests/sync/G.8272/time-error-in-locked-mode/DPLL-to-PHC/testimpl.py[shared Python implementation], the target of the symbolic link
...
* https://github.com/openshift-kni/vse-sync-tests/blob/main/tests/sync/G.8272/time-error-in-locked-mode/DPLL-to-PHC/PRTC-A/testimpl.py[reference implementation]
* https://github.com/openshift-kni/vse-sync-tests/blob/main/tests/sync/G.8272/time-error-in-locked-mode/DPLL-to-PHC/PRTC-A/config.yaml[configuration file]

Each leaf `config.yaml` (e.g. `PRTC-A/config.yaml`) should include a top-level `display_name` string with the full human-readable test title. The `testimpl.py` reads `display_name` from the config at runtime and includes it in the JSON result at key `pdf_display_name`. `testdrive.junit.create` uses `pdf_display_name` from each test result line when building JUnit/PDF names; if absent it falls back to the raw test id.
...
* optionally including a pair at key `"pdf_display_name"` with a value of:
** a string, the same human-readable base title as module-level `PDF_DISPLAY_NAME` in `testimpl.py` (JUnit appends variant and `[card-N]` suffixes from the test id when applicable)
...
https://github.com/openshift-kni/vse-sync-tests/blob/main/tests/sync/G.8272/time-error-in-locked-mode/1PPS-to-DPLL/PRTC-A/testimpl.py[reference implementation]
(which is actually a symbolic link to this
https://github.com/openshift-kni/vse-sync-tests/blob/main/tests/sync/G.8272/time-error-in-locked-mode/1PPS-to-DPLL/testimpl.py[shared Python implementation]).
...
link:../collection_tools[collection_tools].
...
* https://github.com/openshift-kni/vse-sync-tests/blob/main/tests/sync/G.8272/time-error-in-locked-mode/1PPS-to-DPLL/examples/dpll-PRTCA-accept.dat[test success for PRTC-A only]
* https://github.com/openshift-kni/vse-sync-tests/blob/main/tests/sync/G.8272/time-error-in-locked-mode/1PPS-to-DPLL/examples/dpll-reject.dat[test failure]
